...
function setMediaInfo (content_url, content_auth) {
  ...
  media_url = encodeURIComponent(content_url);
}
...
function getMediaURL (isSecure) {
  ...
  if (isSecure == true) {
    mediafullurl = proxy_https_host + '/cddr_streaming?url=' + media_url + '&param=' + media_auth;
  } else {
    mediafullurl = proxy_http_host + '/cddr_streaming?url=' + media_url + '&param=' + media_auth;
  }
...
function launchProxy (uri, successCallback, noHandlerCallback, unknownCallback) {
  ...
  function AgentCheckCallback (result) {
    console.log('AgentCheckCallback called');

    if (result == true) {
      console.log('AgentCheckCallback success');
      callback(successCallback);
    } else {
      console.log('AgentCheckCallback fail');
  ...
  if ((browser.isChrome && !isMac) || browser.isEdge) { //
    blurHandler = function () {
      ...
      console.log('test1 oooooooooooooo');
    ...
    timeoutHandler = function () {
      ...
      console.log('test1 zzzzzzzzzzzz');
    };
...
function indicateRunFail () {
  var html = '<div id=\'area_meg\' style=\'display:table; width:100%;height:100%;\'><p id=\'pmsg\' style=\'display:table-cell; color:#7d7d7d;text-align:center;vertical-align:middle\'>';
  html = html + '플레이어 구동에 실패 하였습니다.<br>페이지를 새로고침 해 주세요.';
  html = html + '</p></div>';
  $('#video').empty().append(html);
}
